...
template <typename S, typename A> struct DefaultExplorationTracker {
  // --- Result Data ---
  ...
  template <typename Node> void on_expand(const Node *node) {
    expanded_nodes++;
    if (node->depth > max_depth)
      max_depth = node->depth;
  }

  template <typename Node> void on_generate(const Node *) { generated_nodes++; }

  void on_frontier_update(std::size_t size) {
    if (size > max_frontier_size)
      max_frontier_size = size;
  }

  template <typename Node> void on_finish(const Node *goal_node) {
